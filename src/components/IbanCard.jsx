    ...
    <div className="card-modern p-2 sm:p-4 md:p-8 transition-all duration-300 mobile-iban-card w-full shadow-modern">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div className="flex-1">
            ...
            <div className="flex-1">
              ...
              {iban.bankName && (
                <div className="flex items-center gap-1 md:gap-2">
                  ...
                  <span className="px-2 md:px-3 py-1 tag-bank text-xs md:text-sm rounded-full font-medium">
                    {iban.bankName}
                  </span>
                </div>
              )}
          ...
          <div className="bg-gray-100/80 dark:bg-slate-700/50 outline-none rounded-lg md:rounded-xl p-2 md:p-4 mb-2 md:mb-4">
            <div className="flex items-center justify-between">
              <p className="text-gray-700 dark:text-gray-200 font-mono text-sm md:text-lg font-semibold iban-number">
                {!isHidden ? "•••• •••• •••• •••• •••• •••• •••" : iban.iban}
              </p>
              <button
                onClick={() => onToggleVisibility(iban.id)}
                className="ml-2 p-1 !outline-none cursor-pointer text-gray-500 hover:text-gray-700 transition-colors touch-friendly"
                title={!isHidden ? "IBAN'ı göster" : "IBAN'ı gizle"}
              >
                {!isHidden ? (
                  <Eye className="w-4 h-4 outline-none md:w-5 md:h-5" />
                ) : (
                  <EyeOff className="w-4 h-4 outline-none md:w-5 md:h-5" />
                )}
              </button>
            </div>
          </div>
          ...
          {iban.description && (
            <p className="text-gray-600 dark:text-gray-300 text-sm md:text-sm mb-2 md:mb-4 leading-relaxed">
              {iban.description}
            </p>
          )}

          {iban.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 md:gap-2 mb-0 md:mb-4">
              <Tag className="w-3 h-3 md:w-4 md:h-4 text-gray-500 mt-1" />
              {iban.tags.map((tag, index) => (
                <span
                  key={index}
                  className="px-2 md:px-3 py-1 tag-custom text-xs md:text-sm rounded-lg font-medium"
                >
                  {tag}
                </span>
              ))}
